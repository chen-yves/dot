...
local function gen_profile(registry)
  ...
  if not registry[profile] then
    vim.notify(
      ("Profile '%s' does not exist. Falling back to '%s'."):format(profile, default_profile),
      vim.log.levels.WARN
    )
    profile = default_profile
  end
  if profile == "self" and vim.fn.has("nvim-0.12") ~= 1 then
    vim.notify(
      "Profile `self` requires Neovim version greater or equal 0.12.x. Falling back to default profile.",
      vim.log.levels.WARN
    )
    profile = default_profile
  end
...
local function resolve_specs(profile_registry)
  -- specs collector
  local specs = {}

  -- profile specs
  for _, spec in ipairs(profile_registry.specs or {}) do
    table.insert(specs, spec)
  end

  -- addon specs
  for _, addon_name in ipairs(profile_registry.addons or {}) do
    local addon = registry[addon_name]
    if not addon then
      vim.notify(("Addon '%s' not found in registry"):format(addon_name), vim.log.levels.WARN)
    elseif addon.type ~= "addon" then
      vim.notify(("Registry '%s' is not an addon"):format(addon_name), vim.log.levels.WARN)
    else
      for _, spec in ipairs(addon.specs or {}) do
        table.insert(specs, spec)
      end
    end
  end
  ...
  -- patch specs
  for _, patch_name in ipairs(profile_registry.patches or {}) do
    local patch = registry[patch_name]
    if not patch then
      vim.notify(("Patch '%s' not found in registry"):format(patch_name), vim.log.levels.WARN)
    elseif patch.type ~= "patch" then
      vim.notify(("Registry '%s' is not a patch"):format(patch_name), vim.log.levels.WARN)
    else
      for _, spec in ipairs(patch.specs or {}) do
        table.insert(specs, spec)
      end
    end
  end
